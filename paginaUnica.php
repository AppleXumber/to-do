...
$idUser = $_GET["id"];
$mostra = mysqli_query($conexao, "SELECT * FROM usuarios where id = $idUser");
$dados = mysqli_fetch_assoc($mostra);

$nome = $dados["nome"];
$mostraToDos = mysqli_query($conexao, "SELECT * FROM to_do where idUsuario=$idUser");
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
...
<body>
    ...
    <section id="atividades_pendentes">
        <?php
            while($dadosToDos = mysqli_fetch_assoc($mostraToDos)) {
                $nome = $dadosToDos["nome"];
                $id = $dadosToDos["id"];
                $idUsuario = $dadosToDos["idUsuario"];
                $descricao = $dadosToDos["descricao"];
                $dataCadastro = explode("-", $dadosToDos["dataCadastro"]);
                $dataFim = explode("-", $dadosToDos["dataFim"]);
                $conclusao = $dadosToDos["conclusao"];
                $categoria = $dadosToDos["categoria"];
                ...
                echo "
                    <form method='post'";
                        if($conclusao == 1) {
                            echo "action='php/alterarConclusao.php?id=$id&acao=concluido&local=paginaUnica&idUser=$idUser'";
                        } else {
                            echo "action='php/alterarConclusao.php?id=$id&acao=andamento&local=paginaUnica&idUser=$idUser'";
                        }
                    echo " class='atividade'>
                        ...
                        <a class='btn btn-success botaoAlterar' href='alterar.php?id=$id&local=paginaUnica'>Alterar</a>
                    </form>
                ";
            };
        ?>
